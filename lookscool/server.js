...
app.post("/api/restaurants/image", function(req, res) {
  var newImage = req.body;

  db.collection(IMAGE_COLLECTION).insertOne(newImage, function(err, doc) {
    if (err) {
...
app.get("/api/restaurants/allImages", function(req, res) {
  db.collection(IMAGE_COLLECTION).find({}).toArray(function(err, docs) {
    if (err) {
...
app.get("/api/restaurants/images/:id", function(req, res) {
  db.collection(IMAGE_COLLECTION).find({name_ID: req.params.id}).toArray(function(err, docs) {
    if (err) {
      handleError(res, err.message, "Failed to get contacts.");
    } else {
      res.status(200).json(docs);
